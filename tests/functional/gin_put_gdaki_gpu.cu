...
static constexpr uint64_t gdaki_test_req_id = 0x123456789abcdef0ULL;
 ...
 *   *out_req_id:  matches the 64-bit wr_id set below
	...
	if (rank == 0) {
		...
		if (!h_result.done) {
			...
			if (h_result.req_id != gdaki_test_req_id) {
				NCCL_OFI_WARN("R0: 64-bit request ID mismatch");
				local_pass = 0;
			}
		}
	}
