...
/**
 * Per-rail registration of the context's shared signal-only scratch buffer.
 *
...
struct nccl_ofi_gin_gdaki_context {
	...
	/* Per-rail registrations of the shared scratch buffer above. Indexed
	 * by rail id; only [0, effective_rails) are populated. Declared
	 * after scratch_buf so the MRs (closed in gdaki_rail_shared's dtor)
	 * are torn down BEFORE the buffer they cover is freed below — C++
	...
	{
		/* Tear down per-rail MRs before the shared scratch buffer. */
