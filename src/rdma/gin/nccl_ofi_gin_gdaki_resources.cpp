 ...
 * because the endpoint accesses GPU memory. No FI_CONTEXT mode is requested: the
 * device stamps each WQE's request id itself, so efa-direct echoes that id as the
 * completion's op_context and generates a completion for every write.
 */
...
void nccl_ofi_gdaki_gin_domain_t::open_rail(uint16_t rail_id, int dev_id)
{
	...
	rail_state.info = ofi_info_ptr(get_gdaki_info(ref_info));
				   ...
				   size_t ep_addr_len, int total_slots, int nranks)
{
	...
	base.completed_count_dev = local_cntr.gpu_ptr();
}
