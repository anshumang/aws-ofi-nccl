						...
						void **mhandle, void **ginHandle)
{
	 ...
	 * per-rail mr_handles as ONE contiguous block.
	 *
	 ...
	 * is therefore required — if the pointers referenced separate
	 ...
	 *
	 * and point rail_handles[r] INTO this same block. The returned
	 * ginHandle is the pointer array; the kernel reads
	 * ((mr_handle **)win)[dev->rail_id]. Every field the kernel touches
	 * lives in one contiguous host allocation. */
	...
	for (uint16_t r = 0; r < num_rails; r++) {
		...
		/* rail_handles[r] points INTO the same block (after the pointer
		 * array header, at the r-th handle slot). */
		auto *gdaki_handle = reinterpret_cast<nccl_ofi_gin_gdaki_mr_handle *>(
			block + header_size + (size_t)r * handle_size);
		...
		rail_handles[r] = gdaki_handle;
	}

	/* Stash on the mhandle so deregMrSym can free it. The mhandle and
	 * the block share a lifetime by construction. */
	sym->gin_device_handle = block;
